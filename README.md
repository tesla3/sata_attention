...
Each row of matrix $M_p \in \mathbb{R}^{m_p \times p}$ (`M` above) contains the indices to the upper hyper-triangular region of an order $p$ symmetric tensor sorted in ascending order. Each element of $C_p$ (`C` above) is a coefficient. For $p = 0$, $M_0$ is an empty matrix, $C_0 = 1$, and $\Phi_0(\cdot) = 1$, which we handle as a special case in PyTorch. The space and time savings grow rapidly as we increase $p$.
